...
class LoginFormAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    {
        $user=$this->entityManager->getRepository(Users::class)->findOneBy(['email' => $request->request->get('email')]);
        $roles=null;
        if (isset($user))
        $roles=$user->getRoles();
        $credentials = [
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey)
    {
        ...
        if (isset($cre["roles"])) {
            if (strcmp($cre["roles"][0], "ROLE_ADMIN") == 0)
                return new RedirectResponse($this->urlGenerator->generate('app_news_show_user'));
            return new RedirectResponse($this->urlGenerator->generate('app_news_show_user'));
        }
    }
